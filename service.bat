...
:: Устанавливаем настройки по умолчанию при установке
call :game_switch_status
call :ipset_switch_status
...
:: Создаем файл по умолчанию, если его нет (чтобы gamefilter был включен)
...
:: Создаем пустой файл по умолчанию, если его нет (режим "any")
if not exist "%listFile%" (
    >"%listFile%" (
        rem Creating empty file for "any" mode
    )
)
